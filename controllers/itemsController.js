...
const getAllItems = async (req, res) => {
    try {
        const { rows } = await pool.query(`SELECT items.id AS item_id, items.name AS item_name, items.price AS item_price, items.description AS item_description, items.image AS item_image , category.name AS category_name FROM items LEFT JOIN category ON items.category_id = category.id`);
        if (rows.length === 0) {
